class TwitterApi
  ...
  def self.get_my_tweets(username)
    ...
    User.create(name: user.name, twitter_handle: user.screen_name, location: user.location, following: user.friends_count, followers: user.followers_count, description: user.description, tweet_count: user.statuses_count)
  ...
  def self.get_user_friends(username, progress)
    ...
    friends_info.each do |friend|
      User.create(name: friend[:name], twitter_handle: friend[:screen_name], location: friend[:location], following: friend[:friends_count], followers: friend[:followers_count], description: friend[:description], tweet_count: friend[:statuses_count])
      progress.increment
    end
  end
